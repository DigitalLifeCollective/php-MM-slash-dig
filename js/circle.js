queue() 
	.defer(d3.csv, "data/tokens.csv")
	.defer(d3.csv, "data/ID.csv")
	.defer(d3.json, "data/activities.json")
	.await(drawAll);
...
function drawAll(error, ageCSV, idCSV, occupations) {
	...
	var padding = 0,
		...
		height = (mobileSize | window.innerWidth < 768 ? width : window.innerHeight );
	...
	var colorCircle = d3.scale.ordinal()
			.domain([0,1,2,3])
			.range(['#8BD9D1','#5BC9C3','#00B0A0','#027266']);  // '#bfbfbf','#838383','#4c4c4c','#1c1c1c'
	...
	function drawCanvas(chosenContext, hidden) {
		...
		for (var i = 0; i < nodeCount; i++) {
			...
			if(node.ID in dataById) {
				...
				if(node.ID.lastIndexOf(currentID, 0) === 0  & !hidden) {
					...
					if (fontSizeTitle < 5) drawTitle = false;  // FROM 8 JS
					...
					if(drawTitle & showText) {	
						...
						//chosenContext.fillText(commaFormat(node.size)+" Units", nodeX, nodeY + -0.75 * nodeR);
		...
		for (var i = 0; i < nodeCount; i++) {
			...
			//And hide those not close to the parent
			if(node.name == "activities" || (typeof node.parent !== "undefined" & typeof node.children !== "undefined") ) { // added node.name JS
				if( !hidden & showText & $.inArray(node.name, kids) >= 0) { //node.name !== "activities" &  JS
					...
					if (node.name == "activities") drawCircularText(chosenContext, main_title, fontSizeTitle, titleFont, nodeX, nodeY, nodeR, rotationText[counter], 0);
				}//if
				counter = counter + 1;
			}//if
	...
	var currentID = "",
		oldID = "",
		kids = ["activities"]; //needed to check which arced titles to show - only those close to the parent node
			...
			if(node !== nodeOld) {
				...
				if(node) {
					...
					if(typeof node.ID !== "undefined") {
						...
						var nodeX = ((node.x - zoomInfo.centerX) * zoomInfo.scale) + centerX ,
							nodeY = ((node.y - zoomInfo.centerY) * zoomInfo.scale) + centerY + 10 * zoomInfo.scale,
							nodeR = node.r * zoomInfo.scale;
						...
						$(".popover").click(function() { window.open(chat_URL+node.parent.parent.name+"/channels/"+node.parent.name,'_self',false); });
					}//if -> typeof node.ID !== "undefined"
				}//if -> node
			}//if -> node !== nodeOld
	...
	var	showText = true, //Only show the text while you're not zooming //CHANGED FROM true  JS
		textAlpha = 1, //After a zoom is finished fade in the text;
		fadeText = false,
		fadeTextDuration = 50;  //CHANGED FROM 750 JS
	...
	function drawCircularText(ctx, text, fontSize, titleFont, centerX, centerY, radius, startAngle, kerning) {
		...
		if (text == main_title ) { ctx.fillStyle = "rgba(139,217,209," + textAlpha +")"; }  // make top label visible
		else { ctx.fillStyle = "rgba(255,255,255," + textAlpha +")"; }
...
*/
	...
	*/
	...
	d3.select(".legendWrapper").transition().duration(1000).delay(1500).style("opacity", 1);
	*/
