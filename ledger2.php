...
switch ($context) {
  ...
  # Then we can get a list of all users in Mattermost using the acquired token
  ################################################################################
  $url = CHAT_URL."api/v4/teams/1t4h181mupb45f1766795p3pec/posts/search";
...
if ($context != "archive" && $context != "archives") {
    $table = "|Circle|".ucfirst($type)."|Committed? Checked?|Action|
|:--|:--|:--|:--|
";
} else {
...
";
    ...
    |```/dig``` ```activities```/```a```|```@username```|List all activities for a given circle or as sudo @username|```/dig a```|
    |```/dig``` ```activities```/```a```|```@me``` or ```@all```|List all activities across ALL circles for me or all|```/dig a```|
    |```/dig``` ```dashboard```/```d```| |Open dashboard link for this circle.|```/dig d```|
    |**ADD**|
